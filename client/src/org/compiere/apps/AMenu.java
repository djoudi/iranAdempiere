 ...
 * @author victor.perez@e-evolution.com <li>FR [ 1966328 ] New Window Info to
 *         MRP and CRP into View <li>
 *         http://sourceforge.net/tracker/index.php?func
 *         =detail&aid=1966328&group_id=176962&atid=879335 <li>BF[2992649] Issue
 *         in Workflow Activities when the records are ordered <li>
 *         https://sourceforge
 *         .net/tracker/?func=detail&aid=2992649&group_id=176962&atid=879332
 * 
 */
public final class AMenu extends CFrame implements ActionListener,
	...
	public AMenu() {
		...
		treePanel = new VTreePanel(m_WindowNo, true, false); // !editable &
																// hasBar
		...
		// Translate
		Env.setContext(m_ctx, m_WindowNo, "WindowName",
				Msg.getMsg(m_ctx, "Menu"));
	...
	// private DecimalFormat m_memoryFormat =
	 ...
	 * 
	 * <pre>
	 * -mainPanel - centerPane - treePanel - wfActivity - wfPanel - southPanel
	 * 		- infoPanel - bNotes - bTask - memoryBar - wfPanel - progressBar
	 * </pre>
	 * 
	 * @throws Exception
	 */
	void jbInit() throws Exception {
		...
		//This code Added By omidp
		LocaleUtil.applyComponentOrientation(centerPane, treePanel, southPanel, infoPanel, menuBar);
		...
		// Tabs
		centerPane.setFont(centerPane.getFont().deriveFont(
				centerPane.getFont().getSize2D() + 1));
		paPanel = PAPanel.get();
		if (paPanel != null) {
			// centerPane.add(paPanel, Msg.getMsg(m_ctx, "PAPanel"));
			centerPane.addTab(Msg.getMsg(m_ctx, "PAPanel"),
					Env.getImageIcon2("InfoAccount16"), paPanel);
			m_tabMenu++;
			m_tabActivities++;
			m_tabWorkflow++;
		}
		...
		centerPane.addTab(Msg.getMsg(m_ctx, "Menu"),
				Env.getImageIcon2("Home16"), treePanel);
		// centerPane.add(new CScrollPane(wfActivity), Msg.getMsg (m_ctx,
		// "WorkflowActivities") + ": 0");
		centerPane.addTab(Msg.getMsg(m_ctx, "WorkflowActivities") + ": 0",
				Env.getImageIcon2("Assignment16"), new CScrollPane(wfActivity));
		// centerPane.add(new CScrollPane(wfPanel), Msg.getMsg (m_ctx,
		// "WorkflowPanel"));
		centerPane.addTab(Msg.getMsg(m_ctx, "WorkflowPanel"),
				Env.getImageIcon2("WorkFlow16"), new CScrollPane(wfPanel));
	...
	private void createMenu() {
		...
		menuBar.add(mFile);
		AEnv.addMenuItem("PrintScreen", null,
				KeyStroke.getKeyStroke(KeyEvent.VK_PRINTSCREEN, 0), mFile, this);
		AEnv.addMenuItem("ScreenShot", null, KeyStroke.getKeyStroke(
				KeyEvent.VK_PRINTSCREEN, KeyEvent.SHIFT_MASK), mFile, this);
		mFile.addSeparator();
		AEnv.addMenuItem(
				"Logout",
				null,
				KeyStroke.getKeyStroke(KeyEvent.VK_L, Event.SHIFT_MASK
						+ Event.ALT_MASK), mFile, this);
		AEnv.addMenuItem(
				"Exit",
				null,
		...
		if (MRole.getDefault().isAllow_Info_Product()) {
			AEnv.addMenuItem("InfoProduct", null,
		...
		if (MRole.getDefault().isAllow_Info_BPartner()) {
			AEnv.addMenuItem(
					...
					null,
					KeyStroke.getKeyStroke(KeyEvent.VK_I, Event.ALT_MASK
							+ Event.CTRL_MASK), mView, this);
		}
		if (MRole.getDefault().isShowAcct()
				&& MRole.getDefault().isAllow_Info_Account()) {
			AEnv.addMenuItem(
					"InfoAccount",
					null,
					KeyStroke.getKeyStroke(KeyEvent.VK_I, Event.ALT_MASK
		...
		KeyStroke ks = KeyStroke
				.getKeyStroke(KeyEvent.VK_W, KeyEvent.CTRL_MASK);
		this.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
				.put(ks, "ShowAllWindow");
	...
	private int getNotes() {
		String sql = "SELECT COUNT(1) FROM AD_Note "
				...
				+ " AND Processed='N'";
		int retValue = DB.getSQLValue(null, sql,
				Env.getAD_Client_ID(Env.getCtx()), m_AD_User_ID);
		return retValue;
	} // getNotes
	...
	private void gotoNotes() {
		...
		(new AMenuStartItem(m_note_Menu_ID, true, Msg.translate(m_ctx,
				"AD_Note_ID"), this)).start(); // async load
	} // gotoMessage
	...
	private void gotoRequests() {
		...
		(new AMenuStartItem(m_request_Menu_ID, true, Msg.translate(m_ctx,
				"R_Request_ID"), this)).start(); // async load
	} // gotoRequests
	...
	public void updateInfo() {
		...
		String msg = MessageFormat
				.format("{0,number,integer} MB - {1,number,integer}%",
						new Object[] { new BigDecimal(total / 1024),
								new BigDecimal(percent) });
		memoryBar.setString(msg);
		//
		// msg =
		// MessageFormat.format("Total Memory {0,number,integer} kB - Free {1,number,integer} kB",
		msg = Msg.getMsg(m_ctx, "MemoryInfo", new Object[] {
				new BigDecimal(total), new BigDecimal(free) });
		memoryBar.setToolTipText(msg);
		// progressBar.repaint();

		// CarlosRuiz - globalqss - [ 1881285 ] Remove unnecessary calls to
		// System.gc
		// if (percent > 50)
		// System.gc();
		...
		if (DB.isConnected()) {
			...
			bRequests.setText(Msg.translate(m_ctx, "R_Request_ID") + ": "
					+ requests);
			// Memo
			int notes = getNotes();
			...
			centerPane
					.setTitleAt(m_tabActivities,
							Msg.getMsg(m_ctx, "WorkflowActivities") + ": "
									+ activities);
			/*
			 * log.config(msg + ", Processors=" +
			 * Runtime.getRuntime().availableProcessors() + ", Requests=" +
			 * requests + ", Notes=" + notes + ", Activities=" + activities +
	...
	public void updateActivities(int act_length) {
		centerPane.setTitleAt(m_tabActivities,
				Msg.getMsg(m_ctx, "WorkflowActivities") + ": " + act_length);
	} // updateInfo
	 ...
	 * 
	 * @param args
	 *            Array of String arguments (ignored)
	 */
	...
	class InfoUpdater implements Runnable {
		...
		public void run() {
			int sleep = MSysConfig.getIntValue("MENU_INFOUPDATER_SLEEP_MS",
					60000, Env.getAD_Client_ID(Env.getCtx()));
			while (stop == false) {
				updateInfo();
